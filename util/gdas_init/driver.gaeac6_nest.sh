...
if [ $EXTRACT_DATA == yes ]; then
  ...
  case $gfs_ver in
    v12 | v13)
      DATAH=$(sbatch  --parsable --clusters=es --partition=dtn_f5_f6 --constraint=f6 --ntasks=1 -t $WALLT -A $PROJECT_CODE -q ${QUEUE_dtn} -J get_${CDUMP} \
       -o log.data.${CDUMP} -e log.data.${CDUMP} ./get_pre-v14.data.sh ${CDUMP})
      DEPEND="-d afterok:$DATAH"
      if [ "$CDUMP" = "gdas" ] ; then
        DATA1=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_enkf \
         -o log.data.enkf -e log.data.enkf ./get_pre-v14.data.sh enkf)
        DEPEND="-d afterok:$DATAH:$DATA1"
      fi
      ;;
    v14)
      DATAH=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_${CDUMP} \
    ...
    v15)
      ...
      if [ "$CDUMP" = "gfs" ] ; then
        ...
        DATA1=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp1 \
         -o log.data.grp1 -e log.data.grp1 ./get_v15.data.sh grp1)
        DATA2=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp2 \
         -o log.data.grp2 -e log.data.grp2 ./get_v15.data.sh grp2)
        DATA3=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp3 \
         -o log.data.grp3 -e log.data.grp3 ./get_v15.data.sh grp3)
        DATA4=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp4 \
         -o log.data.grp4 -e log.data.grp4 ./get_v15.data.sh grp4)
        DATA5=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp5 \
         -o log.data.grp5 -e log.data.grp5 ./get_v15.data.sh grp5)
        DATA6=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp6 \
         -o log.data.grp6 -e log.data.grp6 ./get_v15.data.sh grp6)
        DATA7=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp7 \
         -o log.data.grp7 -e log.data.grp7 ./get_v15.data.sh grp7)
        DATA8=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_grp8 \
      ...
      ;;
    v16retro)
      DATAH=$(sbatch --parsable --partition=batch --ntasks=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE_dtn} -J get_v16retro \
  ...
  case $gfs_ver in
    v12 | v13)
      ...
      sbatch --parsable --ntasks-per-node=6 --nodes=${NODES} --cpus-per-task=$OMP_NUM_THREADS \
        -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE} -J chgres_${CDUMP} \
        -o log.${CDUMP} -e log.${CDUMP} ${DEPEND} run_pre-v14.chgres.sh ${CDUMP}
      ;;
    v14)
      sbatch --parsable --ntasks-per-node=6 --nodes=${NODES} -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE} -J chgres_${CDUMP} \
...
#      -o log.${CDUMP} -e log.${CDUMP} ${DEPEND} run_v16nest.chgres.sh ${CDUMP}
  ...
  if [ "$CDUMP" = "gdas" ]; then
    ...
    if [ "$gfs_ver" = "v16retro" ]; then
      ...
      sbatch --parsable --ntasks-per-node=12 --nodes=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE} -J chgres_enkf \
        ...
        case $gfs_ver in
          v12 | v13)
              ...
              sbatch --parsable --ntasks-per-node=12 --nodes=1 --cpus-per-task=$OMP_NUM_THREADS \
               -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE} -J chgres_${MEMBER_CH} \
               -o log.${MEMBER_CH} -e log.${MEMBER_CH} ${DEPEND} run_pre-v14.chgres.sh ${MEMBER_CH}
            ;;
          v14)
              sbatch --parsable --ntasks-per-node=12 --nodes=1 -t $WALLT -A $PROJECT_CODE -M c6 -q ${QUEUE} -J chgres_${MEMBER_CH} \
